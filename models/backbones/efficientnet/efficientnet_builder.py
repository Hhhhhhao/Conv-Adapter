...
class EfficientNetBuilder:
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            block = CondConvResidual(**ba, tuning_config=self.tuning_config) if ba.get('num_experts', 0) else InvertedResidual(**ba, tuning_config=self.tuning_config)
        elif bt == 'ds' or bt == 'dsa':
            _log_info_if('  DepthwiseSeparable {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = DepthwiseSeparableConv(**ba, tuning_config=self.tuning_config)
        elif bt == 'er':
            _log_info_if('  EdgeResidual {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = EdgeResidual(**ba, tuning_config=self.tuning_config)
        elif bt == 'cn':
            _log_info_if('  ConvBnAct {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = ConvBnAct(**ba, tuning_config=self.tuning_config)
        else:
