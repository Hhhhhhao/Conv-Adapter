...
from timm.models.layers import create_conv2d, DropPath, make_divisible, create_act_layer, get_norm_act_layer
import sys
sys.path.append('./')
...
class ConvBnAct(nn.Module):
    ...
    def __init__(
        ...
        if 'conv_adapt' in self.tuning_config['method']:
            ...
            self.tuning_module = ConvAdapter(in_chs, out_chs, 
                                             kernel_size=kernel_size, 
                                             padding=(kernel_size - 1)//2,
                                             width=int(in_chs//tuning_config['adapt_size']), 
                                             stride=stride, 
                                             groups=int(in_chs//tuning_config['adapt_size']), 
                                             dilation=1,
                                             act_layer=act_layer)
    ...
    def forward(self, x):
        shortcut = x
        if 'conv_adapt' in self.tuning_config['method']:
            out_adapt = self.tuning_module(x)
        x = self.conv(x)
        x = self.bn1(x)
        if self.has_skip:
